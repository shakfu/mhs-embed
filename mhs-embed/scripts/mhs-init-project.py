...
#
# Build variants (Unix):
#   {project_name}-src       - Source embedding (default)
#   {project_name}-src-zstd  - Source embedding + zstd compression
#   {project_name}-pkg       - Package embedding (fast startup)
#   {project_name}-pkg-zstd  - Package + zstd (recommended for distribution)
...
# ============================================
#
# Builds 4 standalone variants:
#   - {project_name}-src:       source embedding (default)
#   - {project_name}-src-zstd:  source embedding + zstd compression
#   - {project_name}-pkg:       precompiled .pkg embedding (fast startup)
#   - {project_name}-pkg-zstd:  compressed packages (recommended)
...
# {project_name}-src: uncompressed source embedding
...
add_custom_command(
    ...
    DEPENDS ${{MHS_EMBED}}
    COMMENT "Generating mhs_embedded_libs.h (uncompressed source)"
    VERBATIM
)

# {project_name}-src-zstd: compressed source embedding
...
add_custom_command(
    ...
    COMMAND ${{MHS_EMBED}}
        ...
        # TODO: Add --lib flags for your static libraries
    DEPENDS ${{MHS_EMBED}}
    COMMENT "Generating mhs_embedded_zstd.h (compressed source)"
    VERBATIM
)
...
add_custom_command(
    ...
    COMMENT "Building base-${{MHS_VERSION}}.pkg"
    VERBATIM
)
...
add_custom_target({project_name}-local-install DEPENDS ${{LOCAL_MCABAL_STAMP}})

# Generate {project_name}.pkg from project modules
# Note: We clear the .mhscache to ensure fresh compilation when sources change.
# MicroHs caches compiled modules in the working directory. By running from
# CMAKE_SOURCE_DIR (not MHS_DIR), we avoid conflicts with MicroHs's own cache.
set({upper}_PKG ${{CMAKE_CURRENT_BINARY_DIR}}/{project_name}-0.1.0.pkg)
add_custom_command(
    OUTPUT ${{{upper}_PKG}}
    COMMAND ${{CMAKE_COMMAND}} -E rm -rf ${{CMAKE_SOURCE_DIR}}/.mhscache
    COMMAND ${{CMAKE_COMMAND}} -E env "MHSDIR=${{MHS_DIR}}"
        ${{MHS_COMPILER}} -P{project_name}-0.1.0 -i${{CMAKE_CURRENT_SOURCE_DIR}}/src
        {camel}
        -o ${{{upper}_PKG}}
    DEPENDS
        ${{CMAKE_CURRENT_SOURCE_DIR}}/src/{camel}.hs
    WORKING_DIRECTORY ${{CMAKE_SOURCE_DIR}}
    COMMENT "Building {project_name}-0.1.0.pkg"
    VERBATIM
)

# {project_name}-pkg: uncompressed package embedding
...
add_custom_command(
    ...
    COMMAND ${{MHS_EMBED}}
        ...
        --pkg packages/{project_name}-0.1.0.pkg=${{{upper}_PKG}}
        --txt-dir ${{BASE_DIR}}
        --app-modules {project_name}-0.1.0.pkg:{camel}
    ...
    DEPENDS
        ...
        ${{LOCAL_MCABAL_STAMP}}
        ${{{upper}_PKG}}
        ${{CMAKE_CURRENT_SOURCE_DIR}}/{project_name}_ffi.h
    COMMENT "Generating mhs_embedded_pkgs.h (uncompressed packages)"
    VERBATIM
)

# {project_name}-pkg-zstd: compressed package embedding (smallest + fast startup)
...
function(add_{project_name}_standalone_variant TARGET_NAME EMBEDDED_HEADER)
    ...
    # Get the header filename for VFS_EMBEDDED_HEADER
    get_filename_component(EMBEDDED_HEADER_NAME ${{EMBEDDED_HEADER}} NAME)
    ...
    # TODO: Link your libraries
    # target_link_libraries(${{TARGET_NAME}} PRIVATE your_library)
endfunction()
...
# {project_name}-src: source embedding (default standalone behavior)
add_{project_name}_standalone_variant({project_name}-src ${{EMBEDDED_SRC_HEADER}})

# {project_name}-src-zstd: source embedding + zstd compression
add_{project_name}_standalone_variant({project_name}-src-zstd ${{EMBEDDED_SRC_ZSTD_HEADER}} USE_ZSTD)

# {project_name}-pkg: package embedding (fast startup)
add_{project_name}_standalone_variant({project_name}-pkg ${{EMBEDDED_PKG_HEADER}} USE_PKG)

# {project_name}-pkg-zstd: compressed package embedding (smallest + fast startup)
add_{project_name}_standalone_variant({project_name}-pkg-zstd ${{EMBEDDED_PKG_ZSTD_HEADER}} USE_PKG USE_ZSTD)

# Backward compatibility: {project_name}-standalone as alias to {project_name}-src
add_custom_target({project_name}-standalone DEPENDS {project_name}-src)

# Convenience target to build all variants
add_custom_target({project_name}-all
    DEPENDS {project_name} {project_name}-src {project_name}-src-zstd {project_name}-pkg {project_name}-pkg-zstd
)
...
if(NOT WIN32)
install(TARGETS {project_name}-src {project_name}-src-zstd {project_name}-pkg {project_name}-pkg-zstd
    RUNTIME DESTINATION bin
)
endif()
