 ...
 *
 * Required defines (set by project):
 *   -DVFS_EMBEDDED_HEADER="path/to/embedded_header.h"
 *
 * Usage:
 *   # Uncompressed .hs files (simpler, slower startup)
 *   cc -DVFS_EMBEDDED_HEADER="mhs_embedded_libs.h" -c vfs.c -o vfs.o
 *
 *   # Compressed .hs files (smaller binary, slower startup)
 *   cc -DVFS_USE_ZSTD -DVFS_EMBEDDED_HEADER="mhs_embedded_zstd.h" -c vfs.c -o vfs.o
 *
 *   # Precompiled .pkg files (fast startup, larger binary)
 *   cc -DVFS_USE_PKG -DVFS_EMBEDDED_HEADER="mhs_embedded_pkgs.h" -c vfs.c -o vfs.o
 *
 *   # Compressed .pkg files (smallest binary, fast startup)
 *   cc -DVFS_USE_PKG -DVFS_USE_ZSTD -DVFS_EMBEDDED_HEADER="mhs_embedded_pkgs_zstd.h" -c vfs.c -o vfs.o
 *
...
/* Include the project-specific embedded header */
#ifndef VFS_EMBEDDED_HEADER
#error "VFS_EMBEDDED_HEADER must be defined to the path of the embedded files header"
#endif

/* Use preprocessor stringification to include the header */
#define VFS_STRINGIFY(x) #x
#define VFS_INCLUDE(file) VFS_STRINGIFY(file)
#include VFS_INCLUDE(VFS_EMBEDDED_HEADER)
